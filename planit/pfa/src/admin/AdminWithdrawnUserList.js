// src/components/AdminWithdrawnUserList.js
...
// 헤더 + 쿼리 토큰 동시전달(백엔드 pickToken 호환)
...
export default function AdminWithdrawnUserList() {
  ...
  const fetchUsers = useCallback(async () => {
    setLoading(true);
    try {
      const url = withToken(`${API}/admin/open/users/withdrawn?page=${page}&size=${size}`);
      const res = await fetch(url, {
        headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
      });
      if (!res.ok) {
        console.error('탈퇴 회원 목록 응답 실패', res.status, await res.text());
  ...
  const fetchDetail = useCallback(async (uid) => {
    ...
    try {
      const url = withToken(`${API}/admin/open/users/withdrawn/${uid}`);
      const res = await fetch(url, {
        headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
      });
      if (!res.ok) {
        ...
        setDetails(prev => ({
          ...prev,
          [uid]: { _error: res.status === 401 || res.status === 403 ? '권한이 없습니다.' : '상세 조회 실패' }
        }));
        return null;
      }
      const data = await res.json();
        ...
        <table className="admin-users-table">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={3} className="center">탈퇴한 회원이 없습니다.</td></tr>
            ) : users.map(u => (
              <React.Fragment key={u.uid ?? u.id}>
                <tr className="row-clickable" onClick={() => handleRowClick(u.uid)}>
                  ...
                  <td style={{ color: '#cc3d3d', fontWeight: 600 }}>
                        ...
                        {loadingDetailUid === u.uid && !details[u.uid] ? (
                          ...
                          return (
                            <div className="detail-grid">
                              ...
                              <div className="detail-field">
                                <div className="label">가입일</div><div className="value">{formatDate(d.joinedAt || d.regDate)}</div>
                              </div>
                              <div className="detail-field">
                                <div className="label" style={{ color: '#cc3d3d' }}>탈퇴일</div>
                                <div className="value" style={{ color: '#cc3d3d', fontWeight: 600 }}>
